...
public class SectionService {
    ...
    public SectionEntity get(Integer id) {
        return regionRepository.findById(id).orElseThrow(() -> new AppBadException("Item not found"));
    }

    public List<SectionDTO> parseSectionList(String articleFullInfo) {
        try {
            return objectMapper.readValue(articleFullInfo, new TypeReference<>() {});
        } catch (Exception e) {
            throw new RuntimeException("Error parsing section list", e);
        }
    }
